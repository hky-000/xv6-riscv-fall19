...
bd_malloc(uint64 nbytes)
{
  ...
  for(; k > fk; k--) {
    ...
    bit_set_alloc(bd_sizes[k-1].alloc, blk_index(k-1, p) >> 1);
...
bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bit_set_alloc(bd_sizes[k].alloc, bi >> 1);  // free p at size k
...
bd_mark(void *start, void *stop)
{
  ...
  for (int k = 0; k < nsizes; k++) { // 一层一层的进行标记
